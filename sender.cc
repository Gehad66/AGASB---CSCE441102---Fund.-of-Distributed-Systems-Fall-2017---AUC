...
void sender (char * message1, char * message2, char * machine, int port){
	...
	if( (n = sendto(s, message1, strlen(message1), 0, &yourSocketAddress,sizeof(struct sockaddr_in))) < 0) perror("Send 2 failed\n");
	if( (n = sendto(s, message2, strlen(message2), 0, &yourSocketAddress,sizeof(struct sockaddr_in))) < 0) perror("Send 2 failed\n");
